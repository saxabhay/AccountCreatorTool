...
public class All_Account_Create extends JFrame implements ActionListener {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					All_Account_Create account = new All_Account_Create();
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (event.getSource() == createButton) {
			...
			if (mp.equals("UK")) {
				...
				WebElement address1 = driver.findElement(By.id("adr_AddressLine1"));
				actions.moveToElement(address1).click().sendKeys("No. 55 An Jia Lou Lu");
				actions.build().perform();

				WebElement city = driver.findElement(By.id("adr_City"));
				actions.moveToElement(city).click().sendKeys("Beijing");
				actions.build().perform();

				WebElement state = driver.findElement(By.id("adr_StateOrRegion"));
				actions.moveToElement(state).click().sendKeys("Beijing");
				actions.build().perform();

				WebElement zip = driver.findElement(By.id("adr_PostalCode"));
				actions.moveToElement(zip).click().sendKeys("100600");
				actions.build().perform();
